...
import itertools
import torch
from sac_safe_action_size import SAC_Safe
...
parser.add_argument('--safe_path',type=str,default="./results/model_1100.4435116431819.pkl")
...
parser.add_argument('--target_update_interval', type=int, default=90, metavar='N',
...
for i in range(50):
    ...
    while not done:
            ...
            action,_,_,_,_,_,_ = agent.select_action(state,evaluate=True,begin=True)  # Sample random action
...
for i_episode in itertools.count(1):
    ...
    while not done:
        ...
        action,lcb,lcb_safe,lcb_diff,q,q_safe,a_size = agent.select_action(state)  # Sample action from policy
        ...
        writer.add_scalar('uncertainity/action_size', a_size, updates)

        LCB_diff.append(lcb_diff.detach().cpu().numpy()[0])
        Action_size.append(a_size)
    ...
    np.save(os.path.join(model_base_filedir,'uncertainity_safe.npy'), uncertainity_list)
    np.save(os.path.join(model_base_filedir,'reward_safe.npy'), reward_list)
    np.save(os.path.join(model_base_filedir,'steps_safe.npy'), steps)
    np.save(os.path.join(model_base_filedir,'LCB_diff.npy'), LCB_diff)
    np.save(os.path.join(model_base_filedir,'Action_size.npy'), Action_size)
    ...
    if i_episode % 150 == 0 and args.eval is True:
        ...
        for _  in range(episodes):
            ...
            while not done:
                action,_,_,_,_,_,_ = agent.select_action(state, evaluate=True)
