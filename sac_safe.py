...
class SAC_Safe(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        self.safe_value_network = VPrior(n_ensemble=self.n_ensemble, num_inputs=num_inputs, hidden_dim=args.hidden_size).to(
            self.device)
        self.safe_value_target_network = VPrior(n_ensemble=self.n_ensemble, num_inputs=num_inputs,
                                           hidden_dim=args.hidden_size).to(
            self.device)
    ...
    def load_checkpoint_safe(self, ckpt_path, evaluate=False):
        ...
        if ckpt_path is not None:
            ...
            self.safe_value_target_network.load_state_dict(checkpoint['value_target_state_dict'])
